...
window_button.title("Counter")          # set window title to 'counter'

logo = PhotoImage(file="countL.png")
window_button.iconphoto(True,logo)
...
count_button.config(command=counter)                            # Integrating a command which is to callback function
                                                                # called 'counter'
count_button.config(relief="raised",
                    bd=7.5)
count_button.config(font=("Arial",20,"italic"))


label=Label(window_button,text=count,           # Initializing a Label and setting its text to variable 'count' (line 2)
            font=('Monospace',50,"bold")        # button text specifications
            )
